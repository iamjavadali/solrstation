...
sudo docker run --name collabora -t -d -p 9980:9980 --privileged -e username=$user -e password=$pass -e “domain=$domain” -e TZ=$tz -e “server_name=$server_name” -e "aliasgroup1=$aliasgroup1" -e "DONT_GEN_SSL_CERT=" -e "extra_params=--o:ssl.enable=false" --restart always --cap-add MKNOD collabora/code
